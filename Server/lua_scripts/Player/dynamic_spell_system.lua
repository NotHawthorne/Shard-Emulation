...
function CacheSpells()		--Pulls the data from the SQL tables and caches it
	...
	repeat
		...
		Shard_SpellData[SpellID] = {SpellID, SkillID, Type, Comment, Data1, Data2, Data3, Data4, Data5, EffectMult, BaseCooldown, CooldownMult, ExEffect1, ExEffect2, ExEffect3, ExDuration1, ExDuration2, ExDuration3}
...
function On_SpellCast (event, player, spell)
	...
	if (Shard_SpellData[spellEntry]~=nil) then

		local SpellID 		= Shard_SpellData[spellEntry][1]
		local SkillID 		= Shard_SpellData[spellEntry][2]
		local Type			= Shard_SpellData[spellEntry][3]
		local Data1			= Shard_SpellData[spellEntry][5]
		local Data2			= Shard_SpellData[spellEntry][6]
		local Data3			= Shard_SpellData[spellEntry][7]
		local Data4			= Shard_SpellData[spellEntry][8]
		local Data5			= Shard_SpellData[spellEntry][9]
		local EffectMult	= Shard_SpellData[spellEntry][10]
		local BaseCooldown	= Shard_SpellData[spellEntry][11]
		local CooldownMult	= Shard_SpellData[spellEntry][12]
		local ExEffect1		= Shard_SpellData[spellEntry][13]
		local ExEffect2		= Shard_SpellData[spellEntry][14]
		local ExEffect3		= Shard_SpellData[spellEntry][15]
		local ExDuration1	= Shard_SpellData[spellEntry][16]
		local ExDuration2	= Shard_SpellData[spellEntry][17]
		local ExDuration3	= Shard_SpellData[spellEntry][18]
